...
module.exports = function(plugins) {
  ...
  return (sourceRequest, sourceResponse, next) => {
...
const _executePluginHandlers = (type, options) => {
  const plugins = options.plugins;
...
function _executePluginHandler(plugin, type, options) {
  ...
  //return handler name
  const getHandlerName = (isRequest, type) => {
    var handlerType = type ? type + '_' : '';
    var handlerPipeline = isRequest ? 'request' : 'response';
    return 'on' + handlerType + handlerPipeline;
  }
  ...
  return (data, cb) => {
    ...
    try {
      ...
      const handler = getHandlerName(isRequest, type); 
      const handlerFunc = plugin[handler];

      if (handlerFunc && _.isFunction(handlerFunc)) {
        //next function that is passed to each plugin handler.
        const cb = function(e, newData) {
        ...
        //if 3 args must be the older style
        if(handlerFunc.length === 3) { 
          handlerFunc.apply(null, options.sourceRequest, options.sourceResponse, fx);
        } else {
          handlerFunc.apply(null, options.sourceRequest, options.sourceResponse, data, fx);
        }
...
const _configured = (config, property) => {
  if (config.headers) {
